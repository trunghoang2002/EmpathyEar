...
def main():
    ...
    model = AutoModel.from_pretrained(model_args.model_name_or_path, trust_remote_code=True, load_in_4bit=True, device_map="auto")

    # Chuẩn bị model cho k-bit training (đưa về trạng thái hỗ trợ injection adapter)
    model = prepare_model_for_kbit_training(model)

    if model_args.quantization_bit is not None:
        print(f"Quantized to {model_args.quantization_bit} bit")
        model = model.quantize(model_args.quantization_bit).cuda()
